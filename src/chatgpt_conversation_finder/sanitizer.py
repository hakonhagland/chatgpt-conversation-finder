...
class Sanitizer:
    ...
    def remove_tokens_with_single_characters(
        self, tokens: list[str], character_set: str
    ) -> None:
        ...
        tokens[:] = [token for token in tokens if not (
            ...
            all(char in character_set for char in token) and
    ...
    def sanitize_conversations(
        ...
        for id, content in conversations.items():
            ...
            if create_set:
                ...
                conversations_sanitized[id] = tokens
        return conversations_sanitized

    def sanitize_conversations_lower_case(
        self, conversations: dict[str, str]
    ) -> dict[str, set[str]]:
        return self.sanitize_conversations(conversations, lower_case=True, create_set=True)

    def sanitize_conversations_exact_phrase(
        self, conversations: dict[str, str]
    ) -> dict[str, list[str]]:
        return self.sanitize_conversations(conversations, lower_case=False, create_set=False)
