...
class IndexManager:
    def __init__(self, config: Config, init_type: str = "load") -> None:
        ...
        self.raw_conversations = Helpers.load_json(self.json_path)
    ...
    def get_conversation_info(self) -> dict[str, dict]:
    ...
    def load_conversations(self) -> dict[str, set[str]]:
        ...
        try:
            ...
            logging.warning(f"Sanitized conversations file not found at {self.json_path}."
    ...
    def load_id_map(self) -> dict[str, int]:
        ...
        try:
            ...
            logging.warning(f"ID map file not found at {id_map_path}. Creating a new ID map.")
    ...
    def sanitize_conversations(self) -> dict[str, set[str]]:
        ...
        sanitizer = Sanitizer()
        conversations = sanitizer.sanitize_conversations_lower_case(flatten_conversations)
    ...
    def save_conversations(self, conversations: dict[str, set[str]]) -> None:
        ...
        conversations = {k: list(v) for k, v in conversations.items()}
        with open(conversations_path, 'w', encoding='utf-8') as f:
            json.dump(conversations, f, indent=4, sort_keys=True)
